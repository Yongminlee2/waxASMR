...
class BallCatalogTest {
    ...
    @Test
    fun fortyTwoBallsWithUniqueIdsAndNames() {
        assertEquals(42, BallCatalog.all.size)
        assertEquals(42, BallCatalog.all.map { it.id }.toSet().size)
        assertEquals(42, BallCatalog.all.map { it.nameKo }.toSet().size)
    }
    ...
    fun everyBallSoundsDifferentFromEveryOther() {
        ...
        assertEquals("소리가 겹치는 볼이 있음", 42, signatures.toSet().size)
    }

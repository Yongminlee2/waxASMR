...
class PalmPose {
    ...
    fun update(hand: HandLandmarks?, dt: Float) {
        ...
        force = if (delta > 0f && dt > 1e-4f) {
            ...
            (speed * sqrt(squeeze.coerceAtLeast(0f)) * FORCE_GAIN * boost).coerceAtMost(FORCE_MAX)
        } else {
            0f
        }
        previousSqueeze = squeeze
    ...
    private companion object {
         ...
         *
         * BreakModel이 기대하는 힘은 3~12다. 임계(0.22)는 그대로지만 조각별
         * "버티는 힘"의 바닥이 0.30에서 0.13으로 내려가, 같은 손상을 주려면
         * 예전보다 3.4배 센 힘이 들어가야 한다. 모델을 직접 부르는 테스트도
         * 이 눈금을 쓴다.
         *
         * 처음에 2.5로 뒀더니 한 번 쥐는 것만으로 볼이 사라졌다. 쥐는 힘이 구 전체에
         * 걸리도록 바꾸면서 실제로 들어가는 손상이 배로 늘어난 탓이다.
         * 0.55로 낮췄는데도 두세 번 쥐면 다 부서진다고 해서 다시 절반쯤 낮췄고,
         * 접는 세기 가중을 넣으면서 보통 쥐기가 도로 세져 0.17까지 내렸다.
         *
         * 그런데 0.17에서는 첫 쥐기에 아무 일도 일어나지 않았다(0%). 힘이 모자란 게
         * 아니라 조각이 전부 비슷하게 단단해서, 임계를 함께 넘을 때까지 두세 번이
         * 걸린 것이었다. 조각별 편차를 [BreakModel] 쪽에서 크게 벌린 뒤 이 값을
         * 3.4배로 올렸다. 무른 조각은 첫 쥐기에 떨어지고 단단한 조각은 그대로 버틴다.
         */
        const val FORCE_GAIN = 0.578f

        /**
         * 힘 상한. [FORCE_GAIN] 과 같은 배로 올려야 "세게 쥐면 더 부서진다"가 유지된다.
         * 상한만 그대로 두면 보통 쥐기가 이미 상한에 닿아 세기 차이가 사라진다.
         */
        const val FORCE_MAX = 11.9f

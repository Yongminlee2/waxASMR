...
class BreakModel(
    ...
    /**
     * 조각별로 다른 "버티는 힘" 0.30~1.0. 조각 번호에서 결정적으로 나온다 —
     * 무작위면 같은 볼을 다시 만들 때마다 다른 자리가 무르게 된다.
     *
     * 제곱을 씌워 단단한 쪽으로 치우치게 한다. 무른 꼬리(1.0 근처)는 그대로라
     * 첫 쥐기는 예전처럼 시원하게 터지는데, 나머지 대부분이 단단해져서
     * 두세 번째 쥐기부터 천천히 떨어져 나간다. 고르게 퍼뜨리면 초반 몇 번에
     * 우르르 몰려 떨어진다.
     */
